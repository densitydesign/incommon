...
// Hak3r B0Y
function getIncommonData() {
  return fetch(config.data.dir + '/incommon/incommonImagesMini.json')
    .then(r => r.json())
}

let __cachedIncommonData = null
function prepareIncommonData() {
  if (__cachedIncommonData === null) {
    return getIncommonData().then(data => {
      __cachedIncommonData = data
      return data
    })
  }
  return Promise.resolve(__cachedIncommonData)
}

const IncommonFilter = {
  imageSelected: () => true,
}
...
function applyIncommonFilter(provider) {
  ...
  prepareIncommonData().then(images => {
    const imagesOfProvider = images.reduce((all, image) => {
      if (image.content_provider === provider) {
        all[image.localName] = true
      }
      return all
    }, {})
    IncommonFilter.imageSelected = (image) => Boolean(imagesOfProvider[image])
    filters.filterImages()
  })
}
...
Filters.prototype.loadFilters = function() {
  ...
  this.filters.push(IncommonFilter)
